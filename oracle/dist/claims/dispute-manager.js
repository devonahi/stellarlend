...
export class DisputeManager {
    ...
    openDispute(claimId, disputantAddress, reason, evidence = []) {
        ...
        const nonDisputeableStatuses = [
            ClaimStatus.DISPUTED,
            ClaimStatus.CANCELLED,
            ClaimStatus.PAID_OUT,
        ];
        if (nonDisputeableStatuses.includes(claim.status)) {
            throw new DisputeError(`Claim '${claimId}' is in status '${claim.status}' and cannot be disputed`, 'INVALID_STATUS');
        }
        ...
        this.repository.transition(claimId, ClaimStatus.DISPUTED, disputantAddress, `Dispute opened: ${reason}`, { evidence });
    ...
    resolveDispute(claimId, resolution, adminAddress, resolutionNotes) {
        ...
        if (dispute.resolution !== undefined) {
            throw new DisputeError(`Dispute for claim '${claimId}' has already been resolved`, 'ALREADY_RESOLVED');
        }
        ...
        const updatedClaim = this.repository.transition(claimId, newStatus, adminAddress, `Dispute resolved: ${resolution}${resolutionNotes ? ` — ${resolutionNotes}` : ''}`, { resolution, admin: adminAddress });
        if (!updatedClaim) {
    ...
    // ── Helpers ─────────────────────────────────────────────────────────────────
    mapResolutionToStatus(resolution) {
        switch (resolution) {
 ...
 * Factory function.
 */
export function createDisputeManager(repository) {
    return new DisputeManager(repository);
}
//# sourceMappingURL=dispute-manager.js.map