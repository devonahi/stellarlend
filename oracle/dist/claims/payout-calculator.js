...
export class PayoutCalculator {
    ...
    calculate(claim, oracle) {
        ...
        const netPayoutAmount = (afterDeductible * discountFactor) / 1000000n;
    ...
    // ── Private Helpers ─────────────────────────────────────────────────────────
    computeDeductible(cappedAmount) {
        if (this.config.deductiblePercent <= 0)
            return 0n;
        return (cappedAmount * BigInt(Math.round(this.config.deductiblePercent * 1_000))) / 100000n;
    }
    /**
     * Returns a discount multiplier [0.0, 1.0].
     * 0 → no discount; 1 → full discount (zero payout).
     */
    computeConfidenceDiscount(confidence) {
        if (confidence >= this.config.minOracleConfidence)
            return 0;
 ...
 * Factory function.
 */
export function createPayoutCalculator(config) {
    return new PayoutCalculator(config);
}
//# sourceMappingURL=payout-calculator.js.map