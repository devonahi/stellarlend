 ...
 */
import { Account, Keypair, Contract, rpc, TransactionBuilder, Networks, xdr, Address, nativeToScVal, } from '@stellar/stellar-sdk';
...
export class ContractUpdater {
     ...
     */
    async updatePrice(asset, price, timestamp) {
     ...
     */
    async submitPriceUpdate(asset, price, timestamp) {
        const contract = new Contract(this.config.contractId);
        const adminAddress = new Address(this.adminKeypair.publicKey());
        const operation = contract.call('set_asset_price', adminAddress.toScVal(), xdr.ScVal.scvSymbol(asset), nativeToScVal(price, { type: 'i128' }), nativeToScVal(timestamp, { type: 'u64' }));
        ...
        if (rpc.Api.isSimulationError(simulated)) {
            ...
            throw new Error(message.startsWith('Simulation failed:') ? message : `Simulation failed: ${message}`);
        }
    ...
    async healthCheck() {
        ...
        try {
            ...
            try {
                ...
                await this.server.simulateTransaction(new TransactionBuilder(new Account(this.adminKeypair.publicKey(), '1'), {
                    fee: '100',
                    networkPassphrase: this.networkPassphrase,
                })
 ...
 */
export function createContractUpdater(config) {
