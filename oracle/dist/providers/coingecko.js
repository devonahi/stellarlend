 ...
 */
export function createCoinGeckoProvider(apiKey) {
    const tier = getApiTier(apiKey);
    // Demo and Free use the same base URL, only Pro uses pro-api
    const baseUrl = tier === 'pro' ? 'https://pro-api.coingecko.com/api/v3' : 'https://api.coingecko.com/api/v3';
    const config = {
        name: 'coingecko',
        enabled: true,
        priority: 1,
